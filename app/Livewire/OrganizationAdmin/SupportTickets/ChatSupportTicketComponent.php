...
class ChatSupportTicketComponent extends Component
{
    ...
    public function loadSupportTicket()
    {
        ...
        $organizationDoctorIds = \App\Models\User::where('org_id', $user->id)
            ->where('user_type', \App\Enums\UserType::DOCTOR)
            ->pluck('id');

        $this->supportTicket = SupportTicket::with(['user.parentOrganization'])
    ...
    public function updateTicketStatus($status)
    {
        ...
        if ($status === 'resolved' || $status === 'closed') {
            $this->supportTicket->update(['resolved_at' => now()]);
        }
    ...
    {
        return view('livewire.organization-admin.support-tickets.chat-support-ticket-component');
    }
}
