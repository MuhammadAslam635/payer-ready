<aside class="w-64 bg-primary-700 text-white flex flex-col p-4 flex-shrink-0 h-full">
    ...
    <div class="lg:hidden flex justify-end mb-4">
        <button @click="sidebarOpen = false" class="p-2 text-primary-200 hover:text-white hover:bg-primary-600 rounded-lg transition-colors">
        ...
        <div class="bg-primary-600 rounded-lg p-3">
            <div class="flex items-center">
                <div class="bg-primary-500 rounded-full p-2 mr-3 flex-shrink-0">
                    <x-ui.icon name="building-office" class="w-4 h-4 sm:w-5 sm:h-5 text-primary-100" />
                </div>
                <div class="min-w-0 flex-1">
                    <p class="text-xs sm:text-sm font-medium text-white truncate">
                        {{ Auth::user()->organizationStaff->first()?->organization?->business_name ?? 'PayerReady' }}</p>
                    <p class="text-xs text-primary-100 truncate">{{ Auth::user()->user_type }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <nav class="flex-grow space-y-2">
        ...
        <a href="{{ route('home') }}" wire:navigate
            class="flex items-center px-4 py-2.5 rounded-lg text-sm font-medium transition-colors {{ request()->routeIs('home' . '*') ? 'bg-primary-700 text-white' : 'text-primary-200 hover:bg-primary-800 hover:text-white' }}">
            <x-ui.icon name="squares-plus" class="w-4 h-4 sm:w-5 sm:h-5 mr-3 text-primary-100 flex-shrink-0" />
