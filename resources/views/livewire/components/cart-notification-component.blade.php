            ...
            <div class="absolute right-0 top-full mt-2 w-64 bg-white rounded-lg shadow-lg border border-gray-200 z-50 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 pointer-events-none">
                <div class="p-4">
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <span class="text-xs text-gray-500">{{ $cartCount }} {{ $cartCount === 1 ? 'item' : 'items' }}</span>
                    </div>
                    
                    <div class="space-y-2 mb-3">
                        @foreach(Cart::instance($cartInstance)->content() as $item)
                            <div class="flex justify-between items-center text-xs">
                                <span class="text-gray-700 truncate flex-1 mr-2">{{ $item->name }}</span>
                    ...
                    <div class="border-t border-gray-200 pt-2">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-sm font-bold text-indigo-600">${{ number_format($cartTotal, 2) }}</span>
                        </div>
                    </div>
                </div>
            </div>
