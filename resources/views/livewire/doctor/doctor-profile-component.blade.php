        ...
        <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
            <div class="text-center">
                ...
                <div class="flex items-center justify-center mt-4">
                    <span class="inline-block px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                        {{ Auth::user()->is_active ? 'Active' : 'Pending' }}
                    </span>
                </div>
            ...
            <div class="mt-6 space-y-3">
                <div class="flex justify-between">
                        ...
                        <span class="text-sm font-medium text-slate-900">{{ Auth::user()->middle_name ?? 'N/A' }}</span>
                    </div>
                @if (Auth::user())
                    @if (Auth::user()->npi_number)
                        <div class="flex justify-between">
                            <span class="text-sm text-slate-500">NPI:</span>
                            <span class="text-sm font-medium text-slate-900">{{ Auth::user()->npi_number }}</span>
                        </div>
                    @endif
                    <div class="flex justify-between">
                        <span class="text-sm text-slate-500">DEA Number:</span>
                        <span class="text-sm font-medium text-slate-900">{{ Auth::user()->dea_number ?? 'N/A' }}</span>
                    </div>
                    @if (Auth::user()->dea_expiration_date)
                        <div class="flex justify-between">
                            <span class="text-sm text-slate-500">Expiration At:</span>
                            <span class="text-sm font-medium text-slate-900">{{ Auth::user()->dea_expiration_date }}
                            </span>
                        </div>
                    @endif
                    <div class="flex justify-between">
                        <span class="text-sm text-slate-500">Date Of Birth:</span>
                        <span class="text-sm font-medium text-slate-900">{{ Auth::user()->date_of_birth ?? 'N/A' }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-sm text-slate-500">Contact Number:</span>
                        <span class="text-sm font-medium text-slate-900">{{ Auth::user()->phone ?? 'N/A' }}</span>
                    </div>
                @endif
                <div class="flex justify-between">
                    <span class="text-sm text-slate-500">Member Since:</span>
                    <span
                        class="text-sm font-medium text-slate-900">{{ auth()->user()->created_at->format('M Y') }}</span>
                </div>
            </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-4">
                <div class="flex items-center">
                    ...
                    <div class="ml-3">
                        <p class="text-2xl font-bold text-slate-900">{{ auth()->user()->certificates->count() ?? 0 }}
                        </p>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-4">
                <div class="flex items-center">
                    ...
                    <div class="ml-3">
                        <p class="text-2xl font-bold text-slate-900">{{ auth()->user()->tasks->count() ?? 0 }}</p>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-4">
                <div class="flex items-center">
                    ...
                    <div class="ml-3">
                        <p class="text-2xl font-bold text-slate-900">{{ auth()->user()->documents->count() ?? 0 }}</p>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-4">
                <div class="flex items-center">
                    ...
                    <div class="ml-3">
                        <p class="text-2xl font-bold text-slate-900">{{ auth()->user()->addresses->count() ?? 0 }}</p>
                        ...
                        @foreach (auth()->user()->transactions->take(5) as $transaction)
                            <div
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        <p class="text-sm font-medium text-slate-900">
                                            {{ $transaction->description ?? 'Transaction' }}</p>
                                ...
                                <div class="text-right">
                                    ...
                                    <p class="text-xs text-slate-500">{{ ucfirst($transaction->status) }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
                    ...
                    @if (auth()->user()->certificates && auth()->user()->certificates->count() > 0)
                        <table class="min-w-full divide-y divide-slate-200">
                            ...
                            <tbody class="bg-white divide-y divide-slate-200">
                                @foreach (auth()->user()->certificates->take(5) as $certificate)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900">
                                            {{ $certificate->certificateType->name ?? 'Certificate' }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                                            {{ $certificate->issue_date ? $certificate->issue_date->format('M d, Y') : '-' }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                                            {{ $certificate->expiry_date ? $certificate->expiry_date->format('M d, Y') : '-' }}
                                        </td>
                ...
                <div class="overflow-x-auto">
                    @if (auth()->user()->documents && auth()->user()->documents->count() > 0)
                        <table class="min-w-full divide-y divide-slate-200">
                            ...
                            <tbody class="bg-white divide-y divide-slate-200">
                                @foreach (auth()->user()->documents->take(10) as $document)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900">
