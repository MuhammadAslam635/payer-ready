                    ...
                    <div class="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                        <div class="flex items-center">
                            ...
                            <div class="ml-3">
                                ...
                                <p class="text-xs text-slate-500">{{ $transaction['user_name'] }} • {{ $transaction['created_at']->diffForHumans() }}</p>
                            </div>
                        </div>
            ...
            <div class="space-y-4">
                @forelse($stats['latestUsers'] as $user)
                    <div class="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                        <div class="flex items-center">
                            <div class="flex-shrink-0">
                                <img class="w-8 h-8 rounded-full object-cover bg-slate-200" src="{{ $user['profile_photo_url'] }}" alt="{{ $user['name'] }}">
                            </div>
                            <div class="ml-3">
                                <p class="text-sm font-medium text-slate-900">{{ $user['name'] }}</p>
                                <p class="text-xs text-slate-500">{{ ucfirst(str_replace('_', ' ', $user['user_type'])) }} • {{ $user['created_at']->diffForHumans() }}</p>
                            </div>
                        </div>
                        <div class="text-right">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200">
                ...
                <tbody class="bg-white divide-y divide-slate-200">
                    @forelse($stats['latestCertificateRequests'] as $request)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <img class="w-8 h-8 rounded-full object-cover bg-slate-200" src="{{ $request['doctor']['profile_photo_url'] }}" alt="{{ $request['doctor']['name'] }}">
                                    <div class="ml-3">
                                        <p class="text-sm font-medium text-slate-900">{{ $request['doctor']['name'] }}</p>
                                        <p class="text-xs text-slate-500">{{ $request['doctor']['email'] }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <p class="text-sm text-slate-900">{{ $request['certificate_type'] }}</p>
                                <p class="text-xs text-slate-500">{{ $request['organization_name'] }}</p>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $request['status'] === 'pending' ? 'bg-yellow-100 text-yellow-800' : ($request['status'] === 'approved' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800') }}">
